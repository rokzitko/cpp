...
public:
    ...
    // For unit tests: run a command string and capture response
...
protected:
    ...
    void load_commands(const std::vector<ScpiCommandDef>& defs) {
        for (auto& def : defs) add_node(def.path, def.set_handler, def.query_handler);
    }
    ...
    void write(const std::string &msg) {
        if (msg.empty()) return;
        last_response_ = msg; // capture last response for test
        // networking write omitted in test context
        auto self(shared_from_this());
        auto out = std::make_shared<std::string>(msg + "\n");
        boost::asio::async_write(socket_, boost::asio::buffer(*out),
            [this, self, out](boost::system::error_code, std::size_t) {});
